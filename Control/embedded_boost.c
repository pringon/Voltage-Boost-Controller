...
int main(void)
{
    ...
    //printf( "%04d:  ", cnt );
		...
		printf("b");
		v_curr = v_load()*R;
		error_margin = (v_target - v_curr)/v_target;
		duty_cycle = (v_curr > v_target+0.2 || v_curr < v_target-0.2) ?
																		(duty_cycle += KP*error_margin) : duty_cycle;
		printf("%0.3f\t", duty_cycle); //Duty Cycle
		printf("%0.3f\t", v_curr); //Current Voltage

    pwm_duty((uint8_t)duty_cycle);   /* Limited by PWM_DUTY_MAX */

		printf("%0.3f\t", v_target); //Target Voltage
...
void get_voltage() {
	...
	scanf("%c", &check);
	if(check == 's') {
		//printf("Enter the selected voltage: ");
		char entry[50];
		uint8_t i = 0;
		while(entry[i] != 'd') {
			scanf("%c", &entry[i]);
			if((entry[i] >= '0' && entry[i] <= '9') || entry[i] == '.') {
				//printf("%c", entry[i]);
				i++;
			}
		}
